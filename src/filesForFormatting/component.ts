...
@Component({
	...
	imports: [FormsModule, ScrollViewComponent, AudioFileCardComponent, CommonModule, DropZoneDirective, AudioFileCardSkeletonComponent, ProgressbarComponent],
})
export class CollectionComponent implements OnInit {
		...
		this._route.queryParams.subscribe((params) => {
			...
			this._audioCollectionsService.getAudioCollectionById(this._collectionId).subscribe({
				next: (audioCollection) => {
					...
					if (this.audioCollection.likedBy === null) {
						this.audioCollection.likedBy = [];
					}
					...
					this.isCollectionLiked = this.audioCollection.likedBy.some((user) => {
		...
		this._audioService.search(this.searchQuery).subscribe(
			{
				next: (audioFiles) => {
					...
					//this.searchedAudioFiles!.push(new Audio({playbackOffset: 0, volume: 0.5}, audioFile, ));
					this.searchedAudioFiles!.push(new Audio(audioFile.id, {audioId: audioFile.id, playbackOffset: 0, volume: 0.5, id:undefined, splinePoints: []}, audioFile));
					});
				},
			});
	...
	public addAudioToCollection(audioFileIdToAdd: string): void {
		...
		this._audioService.getAudioFileById(audioFileIdToAdd).subscribe({
			next: (audioFile) => {
				...
				this.editableAudioCollection.audioList = [...this.editableAudioCollection.audioList, new Audio(audioFileIdToAdd, new AudioMetadata(audioFileIdToAdd, 0.5, 0, [new Point(0, 1, new Tangent(0, 0), new Tangent(0, 0), false), new Point(audioFile.duration / 1000, 1, new Tangent(0, 0), new Tangent(0, 0), false)], temporaryId), audioFile)];
			},
		});
	}
	...
	public deleteAudioFromCollection(audioFileIdToDelete: string): void {
		...
		this.editableAudioCollection.audioList = this.editableAudioCollection.audioList.filter((audioFile) => {
	...
	private _getAudioCollectionUpdateParams(): UpdateAudioCollectionBody {
		...
		if (this.audioCollection.audioList !== this.editableAudioCollection.audioList) {
			...
			this.editableAudioCollection.audioList.forEach((audio) => {
				audioList.push({
	...
	public handleUpdateAudioCollection(confirmed: boolean): void {
		if (confirmed) {
			...
			if (audioCollectionUpdateParams.name === undefined && audioCollectionUpdateParams.audioList === undefined && audioCollectionUpdateParams.description === undefined && audioCollectionUpdateParams.tags === undefined) {
				...
				this._audioCollectionsService.updateAudioCollection(audioCollectionUpdateParams.id, audioCollectionUpdateParams.name, audioCollectionUpdateParams.audioList, audioCollectionUpdateParams.description, audioCollectionUpdateParams.tags).subscribe({
	...
	public handleAudioVolumeChange(updatedAudio: Audio): void {
		if (this.editingMode) {
			this.editableAudioCollection.audioList = this.editableAudioCollection.audioList.map((audio) => {
			...
			this.audioCollection.audioList = this.audioCollection.audioList.map((audio) => {
				if (audio.audioMetadata.id === updatedAudio.audioMetadata.id) {
					return updatedAudio;
				} else {
					return audio;
	...
	private async _decodeAudioFiles(): Promise<CombinedAudioFile[]> {
		...
		await Promise.all(
			this.audioCollection.audioList?.map(async (audioFile, index) => {
				if (audioFile.audio?.fileBytes) {
					console.warn('there are already fileBytes');

					files.push(new CombinedAudioFile(audioFile.audioMetadata.playbackOffset, audioFile.audioMetadata.volume, audioFile.audioMetadata.splinePoints, audioFile.audio?.fileBytes));
				} else {
					...
					files.push(new CombinedAudioFile(audioFile.audioMetadata.playbackOffset, audioFile.audioMetadata.volume, audioFile.audioMetadata.splinePoints, fileBytes));
				}
			}),
		);
		...
		const buffers: CombinedAudioFile[] = await Promise.all(
			files.map(async (file) => {
				...
				return new Promise((resolve, reject) => {
					...
					reader.onload = () => {
						return resolve(new CombinedAudioFile(file.playbackOffset, file.volume, file.splinePoints, undefined, reader.result as ArrayBuffer));
					};
		...
		return await Promise.all(
			buffers.map(
				async (buffer) => {
					return new CombinedAudioFile(buffer.playbackOffset, buffer.volume, buffer.splinePoints,  undefined, undefined, await this._audioCtx.decodeAudioData(buffer.predecodedBuffer!));
				},
			),
		);
	}
			...
			for (let channel = 0; channel < outputBuffer.numberOfChannels; channel++) {
				...
				if (channel < decodedBuffer.numberOfChannels) {
					...
					for (let i = 0; i < outputBuffer.length; i++) {
						...
						outputData[i] += inputData[inputIndex] * buffer.volume;
					}
				}
			}
		});
	...
	public async _loadAudio(buffer: AudioBuffer): Promise<void> {
		...
		await this._wavesurferUtils.waveformFromFile(url, this._waveformContainer).then((waveSurfer) => {
	...
	public incrementAudioCollectionPlays(): void {
		this._audioCollectionsService.incrementAudioCollectionPlays(this.audioCollection.id).subscribe({next: () => {
	...
	public getLoadingStage(): string {
		...
		case 0:
			return '1/6 Uploading files from the server...';
		case 1:
			return '2/6 Converting audio files...';
		case 2:
			return '3/6 Decoding audio files...';
		case 3:
			return '4/6 Combining audio files...';
		case 4:
			return '5/6 Applying audio files to wavesurfer...';
		case 5:
			return '6/6 Ambience loading finished';
		default:
			return '';
		}
	}
	...
	public isCollectionLiked!: boolean;

	public likeAudioCollection(): void {
		this._audioCollectionsService.likeAudioCollection(this.audioCollection.id).subscribe({
			next: () => {
				if (
					this.audioCollection.likedBy.some((user) => {
						return user === this.userId;
					})
				) {
					this.audioCollection.likedBy = this.audioCollection.likedBy.filter((user) => {
						return user !== this.userId;
					});
					this.audioCollection.likes = (this.audioCollection.likes ?? 0) - 1;
				} else {
					this.audioCollection.likedBy.push(this.userId);
					this.audioCollection.likes = (this.audioCollection.likes ?? 0) + 1;
				}
				this.isCollectionLiked = !this.isCollectionLiked;
			},
		});
	}

	public incrementPlays(): void {
		this._audioCollectionsService.incrementAudioCollectionPlays(this.audioCollection.id).subscribe({
			next: () => {
				this.audioCollection.plays! + 1;
			},
		});
	}
}
